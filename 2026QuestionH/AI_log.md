...
## 2026-07-31 增加编码器累计里程与 OLED 观测

### 操作模型
OpenAI Codex（GPT-5）

### 操作时间
2026-07-31 20:37:42 +08:00

### 用户确认
用户已通过 Task1 正常运行并测得一圈的 ODO 数值，确认累计里程与人工停车观测可用。

### 操作内容
- 在编码器 20 ms 锁存中断中累计双轮有符号脉冲，避免 OLED 刷新导致里程丢失。
- 底盘状态增加左右轮累计计数与车体中心平均 ODO。
- Task1、Task3、Task4 运行页和结果页同时显示实时计时与 ODO。
- 支持运行中按 KEY3 人工停车并保留最终 ODO。
- 删除原四路黑线自动停车逻辑，并增加 RAM 中的 ODO 停车阈值设置页面。
